...
class Graph () :
    ...
    def create_oriented(self):
        ...
        dico_valeur: contient la valeur des arètes, dans la cas présent 0 ou 1
        ...
        for k in range(len(L)):
            ...
            dico_valeur[(t1,t2)]=1
            dico_valeur[((t2,t1))]=0
            dico_valeur[((t2,(-2,-2)))]=1
            dico_valeur[(((-1,-1),t1))]=1
        ...
        return (dico_graphe,dico_valeur)
    ...
    def parcours(self,dico_graphe,dico_valeur):
        """Parcours en largeur pour la méthode des flots
        On cherche un chemin entre (-1,-1) le sommet source et (-2,-2) le sommet target.
        Le parcours en largeur est classique si ce n'est qu'il faut vérifier que la valeur
        ...
        while len(a_parcourir)>0:
            ...
            for k in range(len(dico_graphe[sommet])):
                if dico_graphe[sommet][k]==(-2,-2) and dico_valeur[(sommet,dico_graphe[sommet][k])]==1:
                    return Graph.trajet(self, parent,sommet)
                else:
                    if dico_graphe[sommet][k] not in parent and dico_valeur[(sommet,dico_graphe[sommet][k])]==1:
